...
프로필 화면에서 작성 글과 좋아요한 글을 나눠 볼 수 있으며, 팔로우 관계와 저장한 게시글 목록도 함께 관리할 수 있습니다.
...
- **React 19**: 서버 컴포넌트와 최신 클라이언트 패턴을 안정적으로 활용하기 위해 사용합니다.
- **TypeScript**: 컴파일 타임에 오류를 잡고, 코드의 안정성과 유지보수성을 높이기 위해 사용합니다.

### 2-2. Styling & UI

- **Tailwind CSS v4 & shadcn/ui**: 디자인 시스템의 일관성을 유지하면서도 높은 개발 생산성을 위해 도입했습니다.
- **Radix UI**: 접근성 있는 UI 프리미티브를 기반으로 공통 컴포넌트를 구성합니다.

### 2-3. Backend & Database

- **Supabase**: PostgreSQL 데이터베이스와 인증, 스토리지 기능을 빠르게 연결하기 위해 사용합니다.
- **Supabase SSR**: App Router 환경에서 서버, 브라우저, 미들웨어 전반의 인증 상태를 일관되게 다룹니다.

### 2-4. Analytics & Quality

- **PostHog / Sentry**: 제품 이벤트 추적과 오류 모니터링을 운영 환경에 맞게 연결합니다.
- **Vitest / Testing Library / ESLint**: 핵심 도메인 로직과 UI 동작을 테스트하고 정적 검사를 수행합니다.
